...
__global__ void linearAlgebraKernel(float *x, float *y, float a, int N) {
    ...
    if (idx < N) {
        printf("Thread %d (Block %d, Thread %d): Processing index %d, x[%d] = %.2f, y[%d] = %.2f, result = %.2f\n",
               threadIdx.x, blockIdx.x, threadIdx.x, idx, idx, x[idx], idx, y[idx], a * x[idx] + y[idx]);
...
void linearAlgebra(float *x, float *y, float a, int N) {
    ...
    // Wait for GPU to finish before copying the result back
